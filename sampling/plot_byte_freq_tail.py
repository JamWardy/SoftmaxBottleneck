...
def plot_relative_byte_frequency_histograms_top_k(model_relative_freqs):
    model_names = ['4', '8', '12', '16', '32', '512']
    num_models = len(model_names)
    num_cols = 3
    num_rows = math.ceil(num_models / num_cols)

    human_freqs = model_relative_freqs.get('human', {})
    top_10_bytes = sorted(human_freqs.items(), key=lambda x: x[1], reverse=True)[:10]
    top_bytes = [b for b, _ in top_10_bytes]
    human_vals = [v for _, v in top_10_bytes]

    max_y = max(
        max(freqs.get(b, 0) for b in top_bytes)
        for freqs in model_relative_freqs.values()
    )

    fig, axes = plt.subplots(num_rows, num_cols, figsize=(10, 6), constrained_layout=True)
    axes = axes.flatten()

    tick_locations = np.arange(0, 10)
    tick_labels = [byte_labels[b] for b in top_bytes]
    ...
    for i, model in enumerate(model_names):
        ...
        freqs = model_relative_freqs.get(model, {})
        values = [freqs.get(b, 0) for b in top_bytes]

        ax.bar(np.arange(10) - offset/2, values, w, label='NanoGPT')
        ax.bar(np.arange(10) + offset/2, human_vals, w, label='Ground Truth')
        ax.set_title(model)
...
def plot_relative_byte_frequency_histograms_bottom_k(model_relative_freqs):
    model_names = ['4', '8', '12', '16', '32', '512']
    num_models = len(model_names)
    num_cols = 3
    num_rows = math.ceil(num_models / num_cols)
    ...
    human_freqs = model_relative_freqs.get('human', {})
    non_zero_human_bytes = [(b, f) for b, f in human_freqs.items() if f > 0]
    bottom_10_bytes = sorted(non_zero_human_bytes, key=lambda x: x[1])[:10]
    bottom_bytes = [b for b, _ in bottom_10_bytes]
    human_vals = [v for _, v in bottom_10_bytes]

    max_y = max(
        max(freqs.get(b, 0) for b in bottom_bytes)
        for freqs in model_relative_freqs.values()
    )
    ...
    tick_locations = np.arange(0, 10)
    tick_labels = [byte_labels[b] for b in bottom_bytes]
    ...
    for i, model in enumerate(model_names):
        ...
        freqs = model_relative_freqs.get(model, {})
        values = [freqs.get(b, 0) for b in bottom_bytes]
...
plot_relative_byte_frequency_histograms_top_k(model_relative_freqs)

plot_relative_byte_frequency_histograms_bottom_k(model_relative_freqs)